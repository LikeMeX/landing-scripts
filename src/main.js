...
function init(arguments, callback) {
  const isPass = checkFieldsRequireFully(arguments.hiddenFieldConfig, arguments.defaultFields, arguments.landingPageType);
...
function checkFieldsRequireFully(hiddenFieldConfig, defaultFieldsWith = [], landingPageType = 'SGC') {
  // ================ static fields =====================
  ...
  let remainingFields;
  if (landingPageType === 'YR') {
    const optionalFieldsYR = ['campaign_info', 'campaign', 'price', 'course', 'orderbumpdetail', 'discountCode', 'channel', 'channel_name', 'bonusdetail', 'type', 'landing_type', 'redirect_url', 'callback_url', 'category', 'email_cf_channel', 'title', 'free_sku', 'gift_item', 'course_upsell', 'upsell_price', 'upsell_detail'];
    ...
    const optionalFieldsSGC = ['orderbumpdetail', 'campaign', 'campaign_info', 'bonusdetail', 'discountCode', 'channel', 'channel_name', 'type', 'landing_type', 'callback_url', 'category', 'email_cf_channel', 'title', 'free_sku', 'gift_item', 'course_upsell', 'upsell_price', 'upsell_detail'];
    ...
    (event) => {
      ...
      for (const feildName of feildNames) {
        if (feildName === 'fullname') {
          ...
          if (formProps.orderbump && formProps.orderbumpdetail && !feildNames.includes('orderbump', 'orderbumpdetail')) {
...
async function createPaymentWith(formData) {
  ...
  Object.keys(redirectQuery).forEach((key) => paymentSuccessRedirectUrl.searchParams.set(key, redirectQuery[key]));

  const courses = formData['course'] ? formData['course'].split(',') : [];

  if (formData['orderbump'] && formData['orderbumpdetail'].length) {
    const orderbumpdetail = formData['orderbumpdetail'] ? formData['orderbumpdetail'].split(',') : [];
...
async function submitPayment(localStorageItems) {
  ...
  if (courses.length) {
    ...
    dataFromLocalStorage['initial_sku'] = `${dataFromLocalStorage['course']}|${dataFromLocalStorage['email']}`;
...
function includeJqueryAddressScript() {
  ...
  scriptTypeahead.src = 'https://earthchie.github.io/jquery.Thailand.js/jquery.Thailand.js/dependencies/typeahead.bundle.js';
    ...
    ($.Thailand = function (o) {
      ...
      function n(e) {
        var r,
          ...
          c = [];
        return (
          e.lookup && e.words && ((t = e.lookup.split('|')), (a = e.words.split('|')), (e = e.data)),
          (r = function (e) {
            return (
      ...
      var l = function (e, t, a) {
        for (var n, i, o, r = 0, c = 0, s = 0, p = (e += '').length, d = (t += '').length, h = 0; h < p; h += 1)
          for (n = 0; n < d; n += 1) {
            for (i = 0; h + i < p && n + i < d && e.charAt(h + i) === t.charAt(n + i); ) i += 1;
            s < i && ((s = i), (r = h), (c = n));
          }
        return (o = s) && (r && c && (o += l(e.substr(0, c), t.substr(0, c), !1)), r + s < p && c + s < d && (o += l(e.substr(r + s, p - r - s), t.substr(c + s, d - c - s), !1))), !1 === a ? o : e === t ? 100 : d < p ? Math.floor((o / p) * 100) : Math.floor((o / d) * 100);
      };
      !(function (a) {
        var e,
        ...
        switch (('json' !== t && 'zip' !== t && (t = o.database.split('.').pop()), t)) {
          ...
          case 'zip':
            ...
            break;
          default:
            throw new Error('Unknown database type: "' + o.database_type + '". Please define database_type explicitly (json or zip)');
        }
      })(function (i) {
        $.Thailand.DB = i;
        var a,
          ...
          e = {
            ...
            suggestion: function (e) {
              return e && e.district ? (e.zipcode && (e.zipcode = ' » ' + e.zipcode), '<div>' + e.district + ' » ' + e.amphoe + ' » ' + e.province + e.zipcode + '</div>') : '<div class="one-px"></div>';
            },
          };
        for (a in o)
          -1 < a.indexOf('$') &&
                ...
                try {
                  a = new JQL(
                    a
                      ...
                      .map(function (e) {
                        return (
                          ((e = JSON.parse(e)).likely = [5 * l(t, e.district), 3 * l(t, e.amphoe.replace(/^เมือง/, '')), l(t, e.province), l(t, e.zipcode)].reduce(function (e, t) {
                            return Math.max(e, t);
                          })),
                          e
                        );
          ...
          -1 < a.indexOf('$') &&
            ...
            o[a]
              .bind('typeahead:select typeahead:autocomplete', function (e, t) {
                for (a in o) (n = a.replace('$', '')), -1 < a.indexOf('$') && o.hasOwnProperty(a) && o[a] && t[n] && o[a].typeahead('val', t[n]).trigger('change');
